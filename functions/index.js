...
exports.onBookingCreated = onValueCreated("/bookingRequests/{bookingId}", async (event) => {
    ...
    try {
        ...
        driversSnapshot.forEach((driverSnapshot) => {
            ...
            // CORRECTED: Check for latitude and longitude as direct properties of the driver object.
            if (driver && driver.latitude != null && driver.longitude != null) {
                ...
                logger.warn(`Driver ${driverSnapshot.key} is online but is missing location data.`);
            }
        });
...
exports.acceptBooking = onCall(async (request) => {
    ...
    try {
        ...
        // Get the accepting driver'''s details to add to the booking.
        const driverSnapshot = await db.ref(`/drivers/${driverId}`).once("value");
        const driverData = driverSnapshot.val();

        // Update the booking with the driver'''s name and vehicle details.
        // This is done outside the transaction for simplicity.
        await bookingRef.update({
            driverName: driverData.displayName || "Your Driver",
            driverVehicleDetails: driverData.vehicleDetails || "Standard Car",
        });
